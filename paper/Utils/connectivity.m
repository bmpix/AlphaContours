...
[E2V,id] = sort(E2V,2); %sorts elements in each row of E2V
[E2V,ia,ic] = unique(E2V,'rows'); %if [C,ia,ic] = unique(A, 'rows'), then  C = A(ia,:) and A = C(ic,:).
edgeSg = id(:,1) - id(:,2); %edge orientation
t2es = [edgeSg(1:nf), edgeSg((nf+1):(2*nf)), edgeSg((2*nf+1):end)]; %edge orientation per triangle
edgeSg = edgeSg(ia);
...
%vrbl = [2, size(E2V,1)+124];
%idx1=idx(1:size(idx)-1);
E2T = reshape(idx', [2, size(E2V,1)])';
E2T(E2T > nf) = 0;
E2T = [E2T, edgeSg, -edgeSg]; %edge per pair of adjacent triangles and its orientation for each of the triangles
