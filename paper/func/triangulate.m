...
if size(holes,1)<size(holes,2) && size(holes,1)~=1
    holes = holes';
end
...
% Triangulate domain.
if ispc
    !triangle.exe -p mesh.poly 
else
    !./triangle/bin/triangle -p mesh.poly 
end
%> /dev/tmp
...
if ispc
    !del -f mesh.poly mesh.1.poly mesh.1.node mesh.1.ele
else
