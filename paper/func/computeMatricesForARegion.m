...
for jSigned=regionBoundaries
    ...
    H_hat(idx:idx+size(all_p_i,1)-1,:) = quadrature_directional_derivative_faster(boundaryCurve,normals,regionBoundaries,openCurve,all_p_i,curveSize);
...
G(1:(n+1):end) = Gdiag;
G = real(G); %not sure why I need that
